...
def get_positive_features(train_path_pos, feature_params):
    ...
    for i in range(len(positive_files)):
        ...
        feat = vlfeat.hog.hog(image, cell_size)
        feats[i] = feat.flatten()
...
def get_random_negative_features(non_face_scn_path, feature_params, num_samples):
    ...
    while i < len(negative_files):
        image = load_image_gray(negative_files[i])
        m, n = image.shape

        if m < n:
            N_crop = int(m/win_size)
            shift = int(0.5*(n - N_crop*win_size))
            C = shift; R = 0;
        else:
            N_crop = int(n/win_size)
            shift = int(0.5*(m - N_crop*win_size))
            C = 0; R = shift;

        for r in range(R,R+N_crop*win_size,win_size):
            for c in range(C,C+N_crop*win_size,win_size):
                im = image[r:r+win_size,c:c+win_size]
    ...
    feats = np.asarray(np.reshape(feats,(count, n_cell*n_cell*31)))
    if num_samples != 0:
...
def train_classifier(features_pos, features_neg, C):
    ...
    svm = LinearSVC(random_state=0, tol=1e-6, loss='hinge', C = C, max_iter = 20000)
    train_set = np.vstack((features_pos, features_neg))
    y = np.block([np.ones(len(features_pos)), (-1)*np.ones(len(features_neg))])
    svm.fit(train_set,y)
...
def train_kneighbors(features_pos, features_neg, N):

    knn = KNeighborsClassifier(n_neighbors=N, weights='distance',algorithm='auto')
    train_set = np.vstack((features_pos, features_neg))
    y = np.block([np.ones(len(features_pos)), (-1)*np.ones(len(features_neg))])
    knn.fit(train_set, y)
...
def mine_hard_negs(non_face_scn_path, svm, feature_params):
    ...
    n_cell = np.ceil(win_size/cell_size).astype('int')
    #feats = np.random.rand(len(negative_files), n_cell*n_cell*31)
    feats = get_random_negative_features(non_face_scn_path, feature_params, 0)
    predictions = svm.predict(feats)
    ind = np.where(predictions > 0)[0]
...
def run_detector(test_scn_path, svm, feature_params, verbose=False):
    ...
    for idx, im_filename in enumerate(im_filenames):
        ...
        scale_min = win_size/min(im_shape[0:1])
        scale_min = round(scale_min, 2)
        cur_confidences = []
        cur_bboxes = []
        scales = np.arange(scale_min, 1.0, 0.05)

        for scale in scales:
            im_res = cv.resize(im, None, fx=scale, fy=scale)
            im_hog = vlfeat.hog.hog(im_res, cell_size)
            feats, boxes = sub_feats(im_hog, cell_size)
            boxes = boxes*cell_size/scale
            #print(boxes)
            if feats.any():
                R = svm.decision_function(feats)
                #R = svm.predict(feats)
                ind = np.where(R > thresh)[0]
                if len(ind) !=  0:
                    #print(R[ind])
                    if len(cur_bboxes) != 0:
                        cur_bboxes = np.vstack((cur_bboxes, boxes[ind]))
                        cur_confidences = np.hstack((cur_confidences,R[ind]))
                    else:
                        cur_bboxes = boxes[ind]
                        cur_confidences = R[ind]
...
def sub_feats(im_hog, cell_size):
     R, C, _ = im_hog.shape
     feats = []
     boxes = []

     count = 0
     for r in range(R-cell_size):
         for c in range(C-cell_size):
             feat = im_hog[r:r+cell_size, c:c+cell_size]
             feats.append(feat.flatten())
             boxes.append([c,r,c+cell_size-1,r+cell_size-1])
             count += 1

     feats = np.asarray(np.reshape(feats,(count, cell_size*cell_size*31)))
     boxes = np.asarray(np.reshape(boxes,(count, 4)))
