...
def create_datasets(data_path, input_size, rgb=False):
  ...
  #train_data_tforms.append(transforms.RandomResizedCrop(size=max(input_size), scale = (0.8, 1)))
  #train_data_tforms.append(transforms.RandomRotation(degrees=10))
...
class SimpleNet(nn.Module):
  ...
  def __init__(self, num_classes, droprate=0.5, rgb=False, verbose=False):
    ...
    self.classifier = nn.Conv2d(in_channels=20, out_channels=num_classes,
...
def create_part2_model(model, num_classes):
  ...
  #pass
  layer_last = nn.Linear(in_features=4096, out_features=num_classes, bias = True)
  #stdv = 1. / math.sqrt(layer_last.weight.size(1))
  layer_last.weight.data.normal_(0,1)
  layer_last.weight.data.mul_(1e-2)
  new_classifier.append(layer_last)
